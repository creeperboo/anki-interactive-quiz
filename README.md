...
- 不打 `*` 就没有正确答案，这张卡不会判分（1.1.3 起没有「答案」字段了）
...
**1.1.3 起三个题型都没有 `答案` 字段**（之前一阵子加过，已经删干净了）：

- **选择题**：正误只看选项行首的 `*`
- **判断题**：对/错由那个勾选框写进 `题目` 末尾的**隐藏标记**（卡片上看不见）：
...
标记万一被手工编辑掉，插件会在下次启动 Anki 时按自己记的账自动补回来。
...
- 弹出候选列表（按共同标签数、新近排序），一行一条技巧，点 **[用这条]** 选一条
- 候选里**不再显示题目**，只显示技巧正文 + 共同标签；**文字一样的技巧只出现一条**
  （空格、换行不同也算同一条），留的是排在最前面的那张卡
- 这张卡还没标签 → 提示你先加标签
...
新字段 **`知识点`**（三个题型都有）。制卡时**一行写一条「标签 -> 链接」**，答完题后卡片上就会出现
一排可点的标签（`📚 相关知识点` 下面），**点哪个标签跳哪个链接**：

```
唐诗 -> https://zh.wikipedia.org/wiki/静夜思          ← 点「唐诗」→ 系统浏览器
格律 -> anki:search:tag:唐诗                           ← 点「格律」→ 卡片浏览器搜索
```

一行一条、**每行都生效**（不只第一行）；标签重复只留第一条。只写目标不写标签的老写法照样有效。
链接是网址就用浏览器打开；其它一律当 **Anki 搜索式**，在卡片浏览器里搜出来。支持这些前缀：
...
制卡时不用手写格式：「知识点」下面有 **[🏷 配置标签链接]**（默认收起），点开就是这张卡的标签
一行一个输入框，每行都有 **[试打开]** 当场验；标签框里刚加的标签点 **[🔄 刷新标签]** 就出来。
